    ...
    def get_critical_path(self, as_item=False):
        ...
        while q:
            ...
            for to_node in path[-1].to_nodes:
                ...
                #hola.append((length+to_node.duration, path+[to_node]))

        copidi = roads.copy()

        """
        Si hay caminos pequeños dentro de los largos, elimina los pequeños
        """
        ...
        for i in range(len(roads)):
            for j in range(i+1,len(roads)):
                ...
                suma = 0
                for k in range(len(roads[i][1])):
                    if roads[i][1][k] == roads[j][1][k]:
                        suma = suma + 1
                if suma == len(roads[i][1]):
                    copidi.remove(roads[i])
                    break
